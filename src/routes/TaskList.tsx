...
const TaskList = () => {
  ...
  return (
    <>
      ...
      {taskList.length > 0 && (
        <ul>
          {taskList.map((task: TaskItem) => <Task id={task.id} title={task?.title} description={task?.description} dueDate={task.dueDate} priority={task.priority} completed={task.completed} key={`task-${task.id}`} />)}
        </ul>
      )}
      {taskList.length === 0 && (
